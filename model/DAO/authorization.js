...
async function authorizeMediaMember(req, res, next) {
  ...
  connect_DB.query(sql, [
    ...
    if (result[0].permission != "Thành viên ban truyền thông" && result[0].permission != "Thành viên ban chủ nhiệm") {
...
async function authorizeContentMember(req, res, next) {
  ...
  connect_DB.query(sql, [
    ...
    if (result[0].permission != "Thành viên ban nội dung" && result[0].permission != "Thành viên ban chủ nhiệm") {
      return res.status(403).json({ message: "Người dùng không có quyền truy cập trang hay tác vụ này!" });
    }
...
async function authorizeLogisticMember(req, res, next) {
  ...
  connect_DB.query(sql, [
    ...
    if (result[0].permission != "Thành viên ban hậu cần" && result[0].permission != "Thành viên ban chủ nhiệm") {
      return res.status(403).json({ message: "Người dùng không có quyền truy cập trang hay tác vụ này!" });
    }
