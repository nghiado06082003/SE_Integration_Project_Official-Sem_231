    ...
    useEffect(() => {
      ...
      .then((response) => {
        if (response.status === 200 && 'borrowList' in response.data) {
          ...
          console.log(JSON.parse(response.data.borrowList))
        }
      })
      .catch((error) => {
        console.error("Error!!!!!!", error);
          ...
          <tbody>
            {borrowedBooks.map((book, index) => (
              <tr key={book.id}>
                ...
                <td className="border px-4 py-2">{book.expected_returned_day ? (book.expected_returned_day).substring(0,10): "Không"}</td>
                <td className="border px-4 py-2">{(book.status === 2 || book.status==4)? "Chưa tới hạn" : "Quá hạn"}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <tbody>
            {returnRequests.map((request, index) => (
              <tr key={request.id}>
                ...
                <td className="border px-4 py-2">{request.student_name}</td>
                <td className="border px-4 py-2">{request.doc_name}</td>
                <td className="border px-4 py-2">{(request.update_date).substring(0, 10)}</td>
                <td className="border px-4 py-2">{request.status === 4 ? "Đúng hạn" : "Quá hạn"}</td>
                <td className="border px-4 py-2">
                  <button className="bg-green-500 text-white px-2 py-1 mr-2" onClick={()=>accept(request.id)}>Đồng ý</button>
                  {request.status!==4 && <button className="bg-red-500 text-white px-2 py-1" onClick={()=>fine(request.id)}>Phạt</button>}
                </td>
              </tr>
            ))}
          </tbody>
...
const LoanHistory = () => {
  ...
  useEffect(() => {
    axios.get("/api/loanManagement/manager/loanHistory")
    .then((response) => {
      if (response.status === 200 && 'loanHistory' in response.data) {
        setLoanHistory(JSON.parse(response.data.loanHistory));
      }
    })
    .catch((error) => {
      console.error("Error!!!!!!", error);
...
const LoanManagement = () => {
    const options = [
      ...
      { id: 2, label: 'Yêu cầu trả sách' },
      { id: 3, label: 'Sách đang cho mượn' },
      // {id: 4, label: 'Lịch sử cho mượn'}
    ];
    ...
    return (
      <div className="container  max-w-screen-xl mx-auto my-8">
        ...
        <div className="mt-8">
          ...
          {selectedOption.id === 2 && <ReturnRequestsTable />}
          {selectedOption.id === 3 && <BorrowedBooksTable />}
        </div>
      </div>
    );
};
